...
function CreateLatext() {
  ...
  const [createLatestPost] = usePostLatestMutation();
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      formData.append("title", latest.name.trim());

      await createLatestPost(formData).unwrap();
      toast.success("Latest Post successfully created", {
        className: "w-[500px] text-green-600",
      });
      setLatest({ name: "", description: "", video: null });
    } catch (err) {
      toast.error(err.data.message, {
        className: "w-[500px] text-red-600",
      });
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="pt-8">
        <Button>Create Latest</Button>
      </div>
    </form>
  );
}

export default CreateLatext;
